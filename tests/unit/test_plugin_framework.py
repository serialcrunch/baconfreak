...
from src.plugins import PluginManager, TabbedPluginManager, plugin_registry
from src.plugins.base import PluginError, PluginInfo
...
class TestPluginManager(unittest.TestCase):
    ...
    def test_create_plugin_invalid_config(self):
        ...
        bad_config = {"interface": -1, "min_rssi": 300}  # Invalid  # Invalid
...
class TestTabbedPluginManager(unittest.TestCase):
    ...
    @patch("os.popen")
    @patch("src.plugins.wifi.plugin.WiFiPlugin._is_valid_wifi_interface")
    @patch("src.plugins.ble.plugin.BLEPlugin._test_interface")
    @patch("src.plugins.ble.plugin.BLEPlugin._is_valid_hci_interface")
    def test_add_multiple_plugins(
        self, mock_ble_valid, mock_ble_test, mock_wifi_valid, mock_popen, mock_socket, mock_geteuid
    ):
        """Test adding multiple plugins to tabbed manager."""
        # Mock running as root
        mock_geteuid.return_value = 0
        mock_ble_valid.return_value = True
        mock_ble_test.return_value = True
