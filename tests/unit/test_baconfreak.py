...
class TestPcapWriters(unittest.TestCase):
    ...
    def test_pcap_writers_success(self, mock_pcap_writer):
        ...
        with pcap_writers(self.known_path, self.unknown_path, self.devices_path) as (
            known,
            unknown,
            devices,
        ):
...
class TestBluetoothScannerInitialization(unittest.TestCase):
    ...
    def test_scanner_initialization_default(
        self, mock_config, mock_logger, mock_detector, mock_company
    ):
    ...
    @patch("src.baconfreak.BaconFreakLogger")
    @patch("src.baconfreak.config")
    def test_scanner_initialization_custom_interface(
        self, mock_config, mock_logger, mock_detector, mock_company
    ):
    ...
    @patch("src.baconfreak.BaconFreakLogger")
    @patch("src.baconfreak.config")
    def test_scanner_initialization_quiet_mode(
        self, mock_config, mock_logger, mock_detector, mock_company
    ):
    ...
    @patch("src.baconfreak.BaconFreakLogger")
    @patch("src.baconfreak.config")
    def test_scanner_initialization_with_scan_config(
        self, mock_config, mock_logger, mock_detector, mock_company
    ):
    ...
    @patch("src.baconfreak.BaconFreakLogger")
    @patch("src.baconfreak.config")
    def test_scanner_initialization_component_failure(
        self, mock_config, mock_logger, mock_detector, mock_company
    ):
...
class TestBluetoothScannerMethods(unittest.TestCase):
    ...
    def setUp(self):
        """Set up test fixtures."""
        with patch("src.baconfreak.CompanyIdentifiers"), patch(
            "src.baconfreak.DeviceDetector"
        ), patch("src.baconfreak.BaconFreakLogger"), patch("src.baconfreak.config") as mock_config:
            mock_config.scan_config = Mock()
    ...
    def test_create_device_table(self):
        ...
        self.scanner.devices = {"aa:bb:cc:dd:ee:ff": device1, "11:22:33:44:55:66": device2}
...
class TestBluetoothScannerPacketProcessing(unittest.TestCase):
    ...
    def test_get_or_create_device_existing(self):
        ...
        packet_info = PacketInfo(
            addr="aa:bb:cc:dd:ee:ff", rssi=-45, data="1234", device_name="Test Device"
        )
    ...
    def test_packet_callback_no_advertising_reports(self, mock_reports):
        ...
        self.scanner._packet_callback(
            mock_packet, mock_known_writer, mock_unknown_writer, mock_devices_writer
        )
    ...
    def test_process_advertising_report_below_rssi_threshold(self):
        ...
        self.scanner._process_advertising_report(
            mock_report, Mock(), mock_known_writer, mock_unknown_writer, mock_devices_writer
        )
...
class TestBluetoothScannerInterface(unittest.TestCase):
    ...
    def test_initialize_bluetooth_interface_success(
        self, mock_scan_enable, mock_cmd_hdr, mock_hci_hdr, mock_socket
    ):
    ...
    @patch("src.baconfreak.HCI_Command_Hdr")
    @patch("src.baconfreak.HCI_Cmd_LE_Set_Scan_Enable")
    def test_initialize_bluetooth_interface_scan_enable_failed(
        self, mock_scan_enable, mock_cmd_hdr, mock_hci_hdr, mock_socket
    ):
...
class TestBluetoothScannerSummary(unittest.TestCase):
    ...
    def test_print_summary_quiet_mode(self):
        ...
        with patch.object(self.scanner, "_print_rich_summary") as mock_rich, patch.object(
            self.scanner, "_print_simple_summary"
        ) as mock_simple:
